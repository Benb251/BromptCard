...
The implementation work is the authorized source and metadata edits listed above (`manifest.json`, `store/INSTALL.md`, `store/LISTING.md`).

`store/BromptCard-<version>.zip` is generated by the existing package-release script. Do not author or copy that zip by hand.

Once those source and metadata edits are complete, proceed to the project verification phase defined in `PROJECT_ADAPTER.md`. Project verification commands must not be weakened, replaced, or skipped.

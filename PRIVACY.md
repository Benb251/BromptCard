...
- Pinterest and its image CDN (`i.pinimg.com`) are included because Pinterest is the default supported site and the extension must fetch the image you select there. Access to every other website is requested as an **optional permission**, per site, only when you choose to enable it.

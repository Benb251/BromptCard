...
// Gemini Gems keep the full output schema in their configured instructions.
// Always send this per-request guard with the attachment so a Gem cannot treat
// a reverse-prompt request as an instruction to render a new image.
export function buildGemAnalysisGuard(target) {
  return [
    "Analyze the attached reference image using your configured instructions.",
    "TEXT ONLY: return the required JSON and nothing else.",
    "Do not generate, render, recreate, edit, transform, preview, or return any image.",
    "Do not invoke image generation or image editing, even when describing an image-generation prompt.",
    ...imageMetaLines(target)
  ].join("\n");
}
